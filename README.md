...
- Only the messages that changed are redrawn while pi streams, so long conversations stay cheap and a text selection is not wiped mid-run. Background tasks travel without their history, and workspace storage is written on a slow beat.
